...
class VisionLanguageClassification:
    ...
    def getConfiguration(self, **scalars):
        ...
        self.strict_classification = scalars.get("strict_classification", "false").lower() in [
            "true",
            "1",
            "t",
            "y",
        ...
        if self.ai_connection_file:
            if os.sep not in self.ai_connection_file:
                raise Exception('Unable to access the path of the file.Note that this model is not currently supported in ArcGIS online or enterprise.')
            else:    
        ...
        if "version" in conn_params:
            ...
            self.api_key = conn_param_v.get("authenticationSecrets", {}).get("token", None)
            self.api_version = conn_param_v.get("serviceProviderProperties", {}).get("api_version", None)
            self.service_provider = conn_param_v.get('serviceProvider', None)
            
            if self.service_provider == "OpenAI":
                self.deployment_name = conn_param_v.get("serviceProviderProperties", {}).get("model_id", None)
            else:
                self.deployment_name = conn_param_v.get("serviceProviderProperties", {}).get("deployment_name", None)
    ...
    def vectorize(self, **pixelBlocks):
        ...
        for image in pixelBlocks['rasters_pixels']:
            ...
            try:
                ...
                labels.append('Unknown') # Handle cases where the LLM does not include the key Classification in its output. Happens very rarely. 
                reasons.append(answer["Reason"])
